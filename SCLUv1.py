...
class SCLU(bt.Strategy):
    ...
    def next(self):
        if not self.position:#checks if we are in a trade- only one position at a time
            ...
            if self.doi_indic[0] < 0 and self.d2oi_indic[0] < -0.002*(self.oi50ma[0]): 
            ...
            if self.doi_indic[0] > -0.01*(self.refoima) or self.d2oi_indic[0] > -0.001*(self.oi50ma[0]):
...
if __name__ == '__main__':
    ...
    dataname=input("enter OHLCVOi filename- "),
    #headers = False,
    #nullvalue=0.0,
